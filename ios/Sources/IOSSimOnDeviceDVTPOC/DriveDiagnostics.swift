...
public actor DriveDiagnostics {
  ...
  private var outputMode: DriveLocationOutputMode = .defaultMode
  ...
  public func start(
    ...
    outputMode: DriveLocationOutputMode = .defaultMode
  ) async {
    ...
    await recorder.record(
      ...
      metadata: [
        ...
        "drive_output_mode": outputMode.rawValue,
  ...
  public func recordSchedulerTick(
    ...
    if let previous = previousExpectedRouteDistanceMeters {
      ...
      effectiveSchedulerSpeed = DriveTraceMetrics.speedMetersPerSecond(
        ...
        elapsedSeconds: previousActualTickOffset.map { max(0, actualTickOffset - $0) }
    ...
    if let previousActualTickOffset {
      ...
      if DriveTraceMetrics.isSchedulerStall(
        intervalMs: interval, targetIntervalMs: targetIntervalSeconds * 1000)
      {
        ...
        )
      {
        ...
        await recordDetector(
          ...
          metadata: [
            ...
            "expected_distance_delta_m": format(
              selectedSpeedMetersPerSecond * targetIntervalSeconds),
            "lifecycle_state": lifecycleState,
  ...
  public func recordCoordinatorUpdateRequested(
    context: DriveTraceContext, writerID: String, connectionGeneration: Int
  ) async {
    ...
    await recorder.record(
      ...
      metadata: coordinatorMetadata(
        context: context, writerID: writerID, connectionGeneration: connectionGeneration)
    )
  }
  ...
  public func recordCoordinatorUpdateEntered(
    ...
    let delay = DriveTraceMetrics.durationMs(
      begin: context.updateRequestMonotonicTime, end: enteredMonotonicTime)
    await recorder.record(
  ...
  public func recordRequestedUpdate(
    ...
    let observedProjection = observed.flatMap { observation -> RouteProjection? in
      route?.nearestProjection(
        to: CLLocationCoordinate2D(latitude: observation.latitude, longitude: observation.longitude)
      )
    }
  ...
  public func recordRichDriveTransportSet(
    context: DriveTraceContext,
    sample: RichDriveSample,
    result: DriveLocationTransportSetResult
  ) async {
    guard result.transportName == DriveLocationOutputMode.richXCUILocationExperimental.displayName
    else { return }
    await recorder.record(
      category: "RICH_DRIVE_LOCATION_SET",
      component: "RichDriveTransport",
      previousState: nil,
      newState: result.ackMonotonicTime == nil ? "sent_without_ack" : "acknowledged",
      message: "rich drive location sample sent",
      metadata: [
        "drive_session_id": sessionID,
        "writer_id": writerID,
        "tick_trace_id": context.tickTraceID,
        "scheduler_sequence": "\(context.tickSequence)",
        "request_sequence": "\(context.requestSequence)",
        "sample_latitude": format(sample.latitude),
        "sample_longitude": format(sample.longitude),
        "sample_speed_mps": format(sample.speedMetersPerSecond),
        "sample_course_deg": format(sample.courseDegrees),
        "sample_altitude_m": format(sample.altitude),
        "sample_horizontal_accuracy_m": format(sample.horizontalAccuracy),
        "sample_vertical_accuracy_m": format(sample.verticalAccuracy),
        "ipc_send_monotonic_timestamp": format(result.sendMonotonicTime),
        "runner_ack_monotonic_timestamp": format(result.ackMonotonicTime),
        "ack_latency_ms": format(result.ackLatencyMs),
        "xctest_stage": "see_gate3_xctest_events",
        "runner_ready_state": result.ackMonotonicTime == nil
          ? "unknown_or_unacked" : "acknowledged",
        "transport_reconnects": "see_reconnect_events",
        "dropped_replaced_samples": "\(result.droppedOrReplacedSamples)",
        "first_failure": "see_first_abnormal_event",
        "stop_reason": "none",
      ]
    )
  }
  ...
  public func recordObservation(
    ...
    if let previous = lastObservedCoordinate, let previousTime = lastObservationMonotonic {
      ...
      observedGeometricSpeed = DriveTraceMetrics.speedMetersPerSecond(
        distanceDeltaMeters: distance, elapsedSeconds: elapsed)
  ...
  public func finalizeSummary() async -> DriveCharacterizationSummary {
    ...
    let summary = DriveCharacterizationSummary(
      ...
      schedulerWakeJitter: DriveTraceMetrics.timingStatistics(
        milliseconds: schedulerJitterMs.map(abs)),
      expectedDistanceDeltaPerTickMeters: DriveTraceMetrics.timingStatistics(
        milliseconds: expectedDistanceDeltasMeters),
      dvtSetDurations: DriveTraceMetrics.timingStatistics(milliseconds: dvtSetDurationsMs),
      coreLocationPropagationLatencies: DriveTraceMetrics.timingStatistics(
        milliseconds: propagationLatenciesMs),
      ...
      percentageOfCLLocationsWithValidSpeed: clLocationCount > 0
        ? Double(validCLLocationSpeedsMps.count) / Double(clLocationCount) * 100 : nil,
      percentageOfCLLocationsWithValidCourse: clLocationCount > 0
        ? Double(validCLLocationCourseCount) / Double(clLocationCount) * 100 : nil,
      meanCLLocationSpeedWhenValid: mean(validCLLocationSpeedsMps),
      foregroundSchedulerIntervals: DriveTraceMetrics.timingStatistics(
        milliseconds: foregroundIntervalsMs),
      backgroundSchedulerIntervals: DriveTraceMetrics.timingStatistics(
        milliseconds: backgroundIntervalsMs),
  ...
  private func appendObservationMetadata(
    ...
    metadata["source_is_simulated_by_software"] = String(
      describing: observation.isSimulatedBySoftware)
    metadata["source_is_produced_by_accessory"] = String(
      describing: observation.isProducedByAccessory)
    if let projection {
  ...
  private func coordinatorMetadata(
    context: DriveTraceContext, writerID: String, connectionGeneration: Int
  ) -> [String: String] {
    [
  ...
  private func dvtMetadata(context: DriveTraceContext?, writerID: String, connectionGeneration: Int)
    -> [String: String]
  {
    [
  ...
  private func summaryMetadata(_ summary: DriveCharacterizationSummary) -> [String: String] {
    [
      ...
      "mean_expected_distance_delta_per_tick_m": format(
        summary.expectedDistanceDeltaPerTickMeters.meanMs),
      "median_expected_distance_delta_per_tick_m": format(
        summary.expectedDistanceDeltaPerTickMeters.medianMs),
      "p95_expected_distance_delta_per_tick_m": format(
        summary.expectedDistanceDeltaPerTickMeters.p95Ms),
      "max_expected_distance_delta_per_tick_m": format(
        summary.expectedDistanceDeltaPerTickMeters.maxMs),
      ...
      "corelocation_mean_propagation_latency_ms": format(
        summary.coreLocationPropagationLatencies.meanMs),
      "corelocation_p95_propagation_latency_ms": format(
        summary.coreLocationPropagationLatencies.p95Ms),
      "corelocation_max_propagation_latency_ms": format(
        summary.coreLocationPropagationLatencies.maxMs),
      ...
      "percentage_cllocations_with_valid_speed": format(
        summary.percentageOfCLLocationsWithValidSpeed),
      "percentage_cllocations_with_valid_course": format(
        summary.percentageOfCLLocationsWithValidCourse),
  ...
  private func trimTraceDictionaries() {
    if latestRequestedBySequence.count > 256 {
      let keys = latestRequestedBySequence.keys.sorted().prefix(
        latestRequestedBySequence.count - 256)
      for key in keys {
