...
import { isDriveTestingFrontendEnabled } from './components/drive-testing/featureFlags'
import { LabMapState } from './components/drive-testing/types'
import WirelessTestingLab from './components/wireless-testing/WirelessTestingLab'
import WirelessTestingLauncher from './components/wireless-testing/WirelessTestingLauncher'
import { isWirelessTestingFrontendEnabled } from './components/wireless-testing/featureFlags'

const experimentalEnabled = import.meta.env.VITE_ENABLE_EXPERIMENTAL_FEATURES === '1'
export const driveTestingFrontendEnabled = isDriveTestingFrontendEnabled(import.meta.env as unknown as Parameters<typeof isDriveTestingFrontendEnabled>[0])
export const wirelessTestingFrontendEnabled = isWirelessTestingFrontendEnabled(import.meta.env as unknown as Parameters<typeof isWirelessTestingFrontendEnabled>[0])
...
function routeDistance(coordinates: LatLon[]): number {
  ...
  return coordinates.slice(1).reduce((total, point, index) => {
    ...
    const dlon = (point.lon - previous.lon) * Math.PI / 180
...
export default function App() {
  ...
  const existingDriveRouteForLab: DriveRouteResponse | null = roadRoute ?? (routeWaypoints.length >= 2 ? {
    ok: true,
    provider: 'manual',
    profile: 'manual',
    cached: false,
    coordinates: routeWaypoints,
    distance_m: routeDistance(routeWaypoints),
    osrm_duration_s: 0,
    message: '',
  } : null)
  const labCompleted = driveTestingMap?.samples.slice(0, Math.max(1, driveTestingMap.currentSample)) ?? []
  const labRemaining = driveTestingMap?.samples.slice(Math.max(0, driveTestingMap.currentSample - 1)) ?? []
  ...
  return (
    <div style={root}>
      <div style={sidebar}>
        ...
        <DriveTestingLauncher enabled={driveTestingFrontendEnabled} onOpen={() => setShowDriveTesting(true)} />
        <WirelessTestingLauncher enabled={wirelessTestingFrontendEnabled} onOpen={() => setShowWirelessTesting(true)} />
      ...
      <div style={mapWrap}>
        <MapContainer center={[37.7749, -122.4194]} zoom={13} style={{ width: '100%', height: '100%' }}>
          ...
          {showDriveTesting && driveTestingMap?.start && <Marker position={[driveTestingMap.start.lat, driveTestingMap.start.lon]} />}
          {showDriveTesting && driveTestingMap?.destination && <Marker position={[driveTestingMap.destination.lat, driveTestingMap.destination.lon]} />}
          {showDriveTesting && driveTestingMap && driveTestingMap.samples.length === 0 && driveTestingMap.route.length >= 2 && (
            <Polyline positions={driveTestingMap.route.map(point => [point.lat, point.lon] as [number, number])} pathOptions={{ color: '#70a5ff', weight: 5 }} />
          )}
          {showDriveTesting && labCompleted.length >= 2 && (
            <Polyline positions={labCompleted.map(point => [point.lat, point.lon] as [number, number])} pathOptions={{ color: '#1fb99a', weight: 6 }} />
          )}
          {showDriveTesting && labRemaining.length >= 2 && (
            <Polyline positions={labRemaining.map(point => [point.lat, point.lon] as [number, number])} pathOptions={{ color: '#7c8796', weight: 4, dashArray: '8 7' }} />
          )}
          {showDriveTesting && driveTestingMap?.currentCoordinate && (
            <CircleMarker center={[driveTestingMap.currentCoordinate.lat, driveTestingMap.currentCoordinate.lon]} radius={9} pathOptions={{ color: '#fff', fillColor: '#1fb99a', fillOpacity: 1, weight: 2 }} />
          )}
        </MapContainer>
        ...
        {showDriveTesting && (
          <DriveTestingLab
            ...
            existingDriveRoute={existingDriveRouteForLab}
            stableDriveStatus={driveStatus}
            onClose={() => { setShowDriveTesting(false); setDriveTestingMap(null) }}
            onMapState={setDriveTestingMap}
          />
        )}
        {showWirelessTesting && (
          <WirelessTestingLab
            selectedLocation={picked}
            onClose={() => setShowWirelessTesting(false)}
          />
        )}
      </div>
